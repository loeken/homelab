...
resource "null_resource" "migrate" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      #"if sudo update-grub | grep 'update-initramfs: Generating /boot/initrd.img-'; then sudo systemctl reboot; fi"
    ]
  }
  depends_on = [
...
resource "null_resource" "pve_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "DEBIAN_FRONTEND=noninteractive sudo apt install -y lvm2 proxmox-ve postfix open-iscsi",
